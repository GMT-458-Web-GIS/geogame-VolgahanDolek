[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/BhShQpq1)

# LAYOUT:
![alt text](https://github.com/GMT-458-Web-GIS/geogame-VolgahanDolek/Sketch.png)


# The Game:
The game has 3  difficulty levels Easy, Medium and Hard.

Each difficulty has 5 questions.

Game will ask where a place is and ask you to guess.

Since it is nearly impossible to pinpoint the location there will be no lives you will gain score based on how close you got to the actual location.

# Js Library
I will be using leaflet.